...
if ($options['help']) {
    $help = "Execute user account sync with external database.
The auth_clcdb plugin must be enabled and properly configured.
...
if (!is_enabled_auth('db')) {
    cli_error('auth_clcdb plugin is disabled, synchronisation stopped', 2);
}

cli_problem('[AUTH DB] The sync users cron has been deprecated. Please use the scheduled task instead.');

// Abort execution of the CLI script if the \auth_clcdb\task\sync_users is enabled.
$task = \core\task\manager::get_scheduled_task('auth_clcdb\task\sync_users');
